...
| #  | file:line | Category | Detail |
|----|-----------|----------|--------|
| 1  | Rating.cs | Refactor | Extract rating validation to a ValueObject for reuse |
...
**A) Inline comments** — group findings by `file:line`. For each unique file+line,
batch all findings at that location into a single comment.
...
For each unique `file:line` group (e.g. 2 findings on `Song.cs:12`):
```
...
If a reference has a file but no line number (e.g. `Rating.cs` with no `:N`),
post an inline file-level comment (omit the `line` field).
...
Post a confirmation: "Posted {n} inline comment(s) and 1 general comment to PR #{pr_number}."
...
- **Do not modify files, branches, or issue state.** Only write PR comments when the user explicitly approves in step 9.5.
