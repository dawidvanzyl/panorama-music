...
public class CompleteRegistrationRequestValidatorTests
{
	...
	private readonly Mock<ICommonPasswordService> _commonPasswordService;

	public CompleteRegistrationRequestValidatorTests()
	{
		_commonPasswordService = new Mock<ICommonPasswordService>();
	...
	public async Task Validate_EmptyInviteToken_ReturnsFailureNamingInviteToken()
	{
		...
		result.ShouldSatisfyAllConditions(
			...
			result => result.Errors.ShouldContain(e => e.PropertyName == nameof(CompleteRegistrationRequest.InviteToken)));
	}
	...
	public async Task Validate_PasswordBelowLengthMinimum_ReturnsFailureNamingPassword()
	{
		...
		result.ShouldSatisfyAllConditions(
			result => result.IsValid.ShouldBeFalse(),
			result => result.Errors.ShouldContain(e => e.PropertyName == nameof(CompleteRegistrationRequest.NewPassword)));
	}
	...
	{
		_commonPasswordService
			.Setup(s => s.ValidateAsync("password123", It.IsAny<CancellationToken>()))
			.ReturnsAsync(false);
