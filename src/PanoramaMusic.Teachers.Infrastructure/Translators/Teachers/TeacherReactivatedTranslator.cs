...
public sealed class TeacherReactivatedTranslator(IAuditContext auditContext, IUserContext userContext) : IAuditEventTranslator
{
	public AuditLane Lane => AuditLane.Transactional;

	public bool CanTranslate(IDomainEvent domainEvent) => domainEvent is TeacherReactivated;

	public AuditEvent Translate(IDomainEvent domainEvent)
	{
		var teacher = ((TeacherReactivated)domainEvent).Teacher;

		return new AuditEvent(
			Guid.NewGuid(),
			DateTime.UtcNow,
			TeacherAuditEventTypes.TeacherReactivated,
			userContext.UserId,
			userContext.Email,
			teacher.TeacherId,
