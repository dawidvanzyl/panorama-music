...
public static class AdminRoutes
{
	...
	{
		var group = app.MapGroup("/api/users").WithTags("Admin").RequireAuthorization("AdminPolicy");

		group.MapGet("/", async (GetUsersHandler handler, CancellationToken ct) =>
		{
		...
		group.MapPost("/{userId:guid}/invite", async (Guid userId, RegenerateInviteTokenHandler handler, CancellationToken ct) =>
		{
